...
class HomeScreen extends StatelessWidget {
  final String filter;
  const HomeScreen({super.key, required this.filter});
    ...
    switch (filter) {
      case 'Today':
        return tasks
            .where((task) =>
                ...
                task.dueDate!.isBefore(startOfTomorrow))
            .toList();
      ...
      case 'Tomorrow':
        return tasks
            .where((task) =>
                task.dueDate != null &&
                task.dueDate!.isAfter(startOfTomorrow) &&
                task.dueDate!
                    .isBefore(startOfTomorrow.add(const Duration(days: 1))))
            .toList();
      case 'This Week':
        return tasks
            .where((task) =>
